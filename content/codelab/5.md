...
Now we'll go through an exercise where we change a piece of content and some of its properties.Let's open up the file `/content/pages/team.md`. This is where the content and config for the team page is stored. Here's what it looks like (with comments explaining each line).
